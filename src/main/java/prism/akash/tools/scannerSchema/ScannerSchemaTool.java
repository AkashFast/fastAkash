            ...
            while (dirs.hasMoreElements()) {
                ...
                if ("file".equals(protocol)) {
                    ...
                    try {
                                ...
                                if ((idx != -1) || recursive) {
                                    ...
                                    if (name.endsWith(".class") && !entry.isDirectory()) {
                                        ...
                                        try {
                                            ...
                                            // .error("添加用户自定义视图类错误
                                            // 找不到此类的.class文件");
                                            logger.error("ScannerSchemaTool:当前class不存在: " + e.getMessage() + " / " + e.getCause().getMessage());
                                        }
                                    }
                                }
                            }
                        ...
                        logger.error("ScannerSchemaTool:jar包获取文件: " + e.getMessage() + " / " + e.getCause().getMessage());
                    }
                }
            }
        } catch (IOException e) {
            logger.error("ScannerSchemaTool:IO流异常: " + e.getMessage() + " / " + e.getCause().getMessage());
        }
